...
async function go() {
  ...
  if (!amt || amt <= 0)        { err.value = 'El monto debe ser mayor a 0'; return }
...
<template>
  <div class="form-wrap">
    <div class="card">
      <h1>Nuevo QR</h1>
      <p class="sub">QR con monto fijo</p>
      <div v-if="err" class="alert">{{ err }}</div>
      <form @submit.prevent="go">
        <div class="field">
          <label for="amount">Monto</label>
          <input id="amount" v-model="amount" type="number" step="0.01" min="0.01" placeholder="100.00" required />
        </div>
...
.qr-preview { display: flex; align-items: center; gap: .4rem; margin-top: .5rem; padding: .4rem .6rem; border-radius: var(--r); background: var(--surf); border: 1px solid var(--ok, #22c55e); font-size: .78rem; }
