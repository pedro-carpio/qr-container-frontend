  ...
  <div class="logo-wrap">
    <div class="card logo-card">
      ...
      <div class="btn-wrap">
        ...
        <button v-if="selectedFile" class="btn btn-p btn-full" :disabled="busy" @click="upload">
...
.logo-label { font-size: .75rem; font-weight: 600; text-transform: uppercase; letter-spacing: .06em; color: var(--mu); margin-bottom: .5rem; }
.logo-display { width: 100%; border: 1px solid var(--bd); border-radius: var(--r); background: var(--surf); display: flex; align-items: center; justify-content: center; padding: 1rem; }
.logo-display--preview { border-style: dashed; border-color: var(--ac); }
.logo-preview { max-width: 180px; max-height: 120px; object-fit: contain; display: block; }
.file-hidden { display: none; }
.hint { font-size: .7rem; color: var(--mu); text-align: center; margin-top: .75rem; }
.alert-ok { background: rgba(34,197,94,.12); color: #22c55e; border: 1px solid rgba(34,197,94,.25); border-radius: var(--r); padding: .625rem .875rem; font-size: .875rem; margin-bottom: .75rem; }
</style>
